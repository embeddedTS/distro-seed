...
class Task:
    ...
    def _staging_env(self, root):
        ...
        for config in self._staging_configs:
            env[f"DS_TASK_WORK_{config}"] = os.path.join(root, config)
        for provides, config in self._staging_provides.items():
            env[f"DS_TASK_WORK_{provides}"] = os.path.join(root, config)
        env["DS_TASK_WORK"] = os.path.join(root, self.config)
        return env
    ...
    def run(self):
        ...
        if self.cmd_type == 'host':
            ...
            os.makedirs(taskenv["DS_TASK_WORK"], exist_ok=True)
            ...
            try:
                ...
                self._stage_host_staging(work, taskenv["DS_TASK_WORK"])
            finally:
