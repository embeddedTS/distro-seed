...
common/host/fetch_git.sh "$GITURL" "$CONFIG_DS_COMPONENT_MURATA_NXP_LINUX_CALIBRATION_GIT_VERSION" "${DS_TASK_WORK}"

install -d "$DS_OVERLAY/lib/firmware/nxp/murata/"
install -d "$DS_OVERLAY/lib/firmware/nxp/murata/files"

install -m 644 "${DS_TASK_WORK}/LICENSE" "$DS_OVERLAY/lib/firmware/nxp/murata/LICENSE"
install -m 644 "${DS_TASK_WORK}/murata/switch_regions.sh" "$DS_OVERLAY/lib/firmware/nxp/murata/"
install -m 644 "${DS_TASK_WORK}/murata/files/bt_power_config_1.sh" "$DS_OVERLAY/lib/firmware/nxp/murata/files/"
install -m 644 "${DS_TASK_WORK}/murata/files/wifi_mod_para_murata.conf" "$DS_OVERLAY/lib/firmware/nxp/murata/files/"

if [ "${CONFIG_DS_COMPONENT_MURATA_NXP_LINUX_CALIBRATION_2DL}" == "y" ]; then
    install -d "$DS_OVERLAY/lib/firmware/nxp/murata/files/2DL"
    install -m 644 "${DS_TASK_WORK}/murata/files/2DL/"* "$DS_OVERLAY/lib/firmware/nxp/murata/files/2DL/"
fi
